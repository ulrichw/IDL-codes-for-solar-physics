; PROGRAM TO FIT FOR THE WAVELENGTH OF THE COTUNES TAKEN WITH THE LASER
...
;-------------------------------------------------
    ...
    WHILE (converg EQ 0) DO BEGIN
        ...
        FOR j=0,nseq-1 DO BEGIN
            ...
            profileg = 1.0      ;profilef
            profilegm= 1.0      ;profilefm
            profilegp= 1.0      ;profilefp
            
            FOR i=0,2 DO profileg  = profileg  * (1.d0+Bg[i]*COS(dpi/FSR[i]*lam0g[iii]+Phig[i]+tuningJS[j,i]))/2.d0
            FOR i=0,2 DO profilegm = profilegm * (1.d0+Bg[i]*COS(dpi/FSR[i]*(lam0g[iii]-0.003)+Phig[i]+tuningJS[j,i]))/2.d0
            FOR i=0,2 DO profilegp = profilegp * (1.d0+Bg[i]*COS(dpi/FSR[i]*(lam0g[iii]+0.003)+Phig[i]+tuningJS[j,i]))/2.d0
            

            Residual[j]   = Inten[j] - profileg*I0g[iii]
        ;PRINT,profileg*I0g,Inten[j]
            dIntendIc[j]  =  profileg
            dIntendlam0[j]=  I0g[iii]*(profilegp-profilegm)/0.006d0
...
; RECONSTRUCTION
    ...
    ENDFOR
    PRINT,list[iii],lam0g[iii]+lamref,lam0[iii]+lamref,SQRT(TOTAL((Intenrecons-Inten)^2.))/SQRT(TOTAL(Intenrecons^2.0)),I0g[iii]
