...
; FOR DATA AFTER 02/22/2007
; PLUS: INCLUDES THE READING OF THE HCM POSITIONS

pro readimages,namelist,images,imx,headers,nbin=nbin,silent=silent,noshow=noshow,raw=raw,getnames=getnames,iover=iover,pad=pad,fixcenter=fixcenter
...
if (type eq 0) then begin ; RAL
  ...
  for i=0,nim-1 do begin
   ...
   ; AJOUT SEBASTIEN
    ...
    if ((not raw) and ((imcfg lt 2) or (imcfg gt 13))) then stop
    im=im and 32767
    if ((imcfg mod 3) eq 2) then begin
      io=0.0
    end
    if ((imcfg mod 3) eq 0) then begin
    ...
    if ((imcfg mod 3) eq 1) then begin
    ...
    sname=strmid(name,ln-10,6)
    iover(i)=io
    if (silent eq 0) then print,i,sxpar(header,'HSQFGSN'),sxpar(header,'SHS') mod 86400,sxpar(header,'HSHIEXP'),sxpar(header,'HCFTID'),sxpar(header,'HWLTID'),sxpar(header,'HPLTID'),rebin(im(0:63,iy(0:63))+0.0,1),io,format='(i4,i7,i6,i6,3i4,2f9.2)'
    headers(i)=ptr_new(header)
    if (noshow eq 0) then tvscl,rebin(images(*,*,i),nsmall,nsmall),i mod nshow
  end
end
...
FOR i=0,nelem-1 DO imx[*,*,i]=imx[*,*,i]-cav0[i]
FOR i=0,nx-1 DO FOR j=0,nx-1 DO imx[i,j,2:nelem-2]=imx[i,j,2:nelem-2]-INTERPOL([imx[i,j,1],imx[i,j,nelem-1]],[jd[1],jd[nelem-1]],jd[2:nelem-2])
