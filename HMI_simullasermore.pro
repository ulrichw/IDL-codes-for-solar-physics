...
; PLUS: WE ALSO FIT FOR DEFECTS IN THE TUNING HALF-WAVE PLATES
;
;
...
Ic           = 5.657d6
Inten0       = FLTARR(nseq)
alpha        = [0.085d0,-0.09d0,0.07d0] ; defect on the retardance of the tuning waveplates
;alpha        = [0.d0,0.d0,0.d0]
gamma        = (1.d0-alpha^2.d0/2.d0)

FOR j=0,nseq-1 DO BEGIN
    profile = blocker0
    FOR i=0,2 DO profile = profile * (gamma[i]*B[i]*COS(FSR[i]/2.d0*(lam0+Phi[i])+tuning[i,j]/2.d0)^2.d0 + 2.d0*(1.d0-gamma[i])*B[i]*COS(FSR[i]/2.d0*(lam0+Phi[i]))^2.d0+(1.d0-B[i])/2.d0)
...
alphag      = DBLARR(3)
gammag      = alphag
nparam      = 7                   ; number of parameters we fit for
nrepet      = 500
...
dIntendAlpha0 = DBLARR(nseq)
dIntendGamma0 = DBLARR(nseq)
dIntendAlpha1 = DBLARR(nseq)
dIntendAlpha2 = DBLARR(nseq)
...
; ADDITION OF PHOTON NOISE AND DARK CURRENT
    ...
    Bg[*]   = contrasts
    Phig[*] = 0.0
    alphag[*] = [0.d0,alpha[1],alpha[2]]
    gammag[*] = (1.d0-alphag[*]^2.d0/2.d0)
   ...
   ;FOR i=3,6 DO profilef = profilef * (1.d0+contrasts*COS(FSR[i]*lam0))/2.d0
    ...
    WHILE (converg EQ 0) DO BEGIN
        ...
        FOR j=0,nseq-1 DO BEGIN
            ...
            NB = gammag[0]*Bg[0]*COS(FSR[0]/2.d0*(lam0+Phig[0])+tuning[0,j]/2.d0)^2.d0+2.d0*(1.d0-gammag[0])*Bg[0]*COS(FSR[0]/2.d0*(lam0+Phig[0]))^2.d0+(1.d0-Bg[0])/2.d0
            WB = gammag[1]*Bg[1]*COS(FSR[1]/2.d0*(lam0+Phig[1])+tuning[1,j]/2.d0)^2.d0+2.d0*(1.d0-gammag[1])*Bg[1]*COS(FSR[1]/2.d0*(lam0+Phig[1]))^2.d0+(1.d0-Bg[1])/2.d0
            E1 = gammag[2]*Bg[2]*COS(FSR[2]/2.d0*(lam0+Phig[2])+tuning[2,j]/2.d0)^2.d0+2.d0*(1.d0-gammag[2])*Bg[2]*COS(FSR[2]/2.d0*(lam0+Phig[2]))^2.d0+(1.d0-Bg[2])/2.d0
            
            Residual[j]      =  Inten[j] - Ic*profilef*NB*WB*E1*dlam

            dIntendPhi0[j]   =  (-gammag[0]*Bg[0]*COS(FSR[0]/2.d0*(lam0+Phig[0])+tuning[0,j]/2.d0)*SIN(FSR[0]/2.d0*(lam0+Phig[0])+tuning[0,j]/2.d0)*FSR[0]-2.d0*(1.d0-gammag[0])*Bg[0]*COS(FSR[0]/2.d0*(lam0+Phig[0]))*SIN(FSR[0]/2.d0*(lam0+Phig[0]))*FSR[0]) * WB * E1 * profilef * Ic * dlam
            dIntendPhi1[j]   =  (-gammag[1]*Bg[1]*COS(FSR[1]/2.d0*(lam0+Phig[1])+tuning[1,j]/2.d0)*SIN(FSR[1]/2.d0*(lam0+Phig[1])+tuning[1,j]/2.d0)*FSR[0]-2.d0*(1.d0-gammag[1])*Bg[1]*COS(FSR[1]/2.d0*(lam0+Phig[1]))*SIN(FSR[1]/2.d0*(lam0+Phig[1]))*FSR[1]) * NB * E1 * profilef * Ic * dlam
            dIntendPhi2[j]   =  (-gammag[2]*Bg[2]*COS(FSR[2]/2.d0*(lam0+Phig[2])+tuning[2,j]/2.d0)*SIN(FSR[2]/2.d0*(lam0+Phig[2])+tuning[2,j]/2.d0)*FSR[0]-2.d0*(1.d0-gammag[2])*Bg[2]*COS(FSR[2]/2.d0*(lam0+Phig[2]))*SIN(FSR[2]/2.d0*(lam0+Phig[2]))*FSR[2]) * NB * WB * profilef * Ic * dlam

            dIntendB0[j]     =  ( gammag[0]*COS(FSR[0]/2.d0*(lam0+Phig[0])+tuning[0,j]/2.d0)^2.d0+2.d0*(1.d0-gammag[0])*COS(FSR[0]/2.d0*(lam0+Phig[0]))^2.d0-0.5d0) * WB * E1 * profilef * Ic * dlam
            dIntendB1[j]     =  ( gammag[1]*COS(FSR[1]/2.d0*(lam0+Phig[1])+tuning[1,j]/2.d0)^2.d0+2.d0*(1.d0-gammag[1])*COS(FSR[1]/2.d0*(lam0+Phig[1]))^2.d0-0.5d0) * NB * E1 * profilef * Ic * dlam
            dIntendB2[j]     =  ( gammag[2]*COS(FSR[2]/2.d0*(lam0+Phig[2])+tuning[2,j]/2.d0)^2.d0+2.d0*(1.d0-gammag[2])*COS(FSR[2]/2.d0*(lam0+Phig[2]))^2.d0-0.5d0) * NB * WB * profilef * Ic * dlam

           ;dIntendAlpha0[j] = (-alphag[0]*Bg[0]*COS(FSR[0]/2.d0*(lam0+Phig[0])+tuning[0,j]/2.d0)^2.d0+2.d0*alphag[0]*Bg[0]*COS(FSR[0]/2.d0*(lam0+Phig[0]))^2.d0) * WB * E1 * profilef * Ic * dlam
            dIntendGamma0[j] = ( Bg[0]*COS(FSR[0]/2.d0*(lam0+Phig[0])+tuning[0,j]/2.d0)^2.d0-2.d0*Bg[0]*COS(FSR[0]/2.d0*(lam0+Phig[0]))^2.d0)* WB * E1 * profilef * Ic * dlam

           ;dIntendAlpha1[j] = (-alphag[1]*Bg[1]*COS(FSR[1]/2.d0*(lam0+Phig[1])+tuning[1,j]/2.d0)^2.d0+2.d0*alphag[1]*Bg[1]*COS(FSR[1]/2.d0*(lam0+Phig[1]))^2.d0)*NB * E1* profilef * Ic * dlam
           ;dIntendAlpha2[j] = (-alphag[2]*Bg[2]*COS(FSR[2]/2.d0*(lam0+Phig[2])+tuning[2,j]/2.d0)^2.d0+2.d0*alphag[2]*Bg[2]*COS(FSR[2]/2.d0*(lam0+Phig[2]))^2.d0)*NB * WB* profilef * Ic * dlam
        ...
        FOR i= 0,nseq-1 DO BEGIN
            ...
            Jac[4,i]  = dIntendB1[i]
            Jac[5,i]  = dIntendB2[i]
            ;Jac[6,i]  = dIntendAlpha0[i]
            Jac[6,i]  = dIntendGamma0[i]
            ;Jac[7,i]  = dIntendAlpha1[i]
            ;Jac[8,i]  = dIntendAlpha2[i]
        ...
        IF(converg EQ 2) THEN BEGIN
            ...
            Bg[1]   = history[j[0],4]
            Bg[2]   = history[j[0],5]
            gammag[0]=history[j[0],6]
            ;alphag[0]=history[j[0],6]
            ;alphag[1]=history[j[0],7]
            ;alphag[2]=history[j[0],8]
        ENDIF
        ...
        IF(converg NE 2) THEN BEGIN
            ...
            ;alphag[0] = alphag[0]+Dx[6]
            ;IF(alphag[0] GT 1.0d0 OR alphag[0] LT 0.0001) THEN alphag[0] = 0.001
            ;alphag[1] = alphag[1]+Dx[7]
            ;IF(ABS(alphag[1]) GT 1.0d0) THEN alphag[1] = 0.01
            ;alphag[2] = alphag[2]+Dx[8]
            ;IF(ABS(alphag[2]) GT 1.0d0) THEN alphag[2] = 0.01
            gammag[0] = gammag[0] +Dx[6]
            IF( gammag[0] GT 1.2d0 OR gammag[0] LT 0.7) THEN gammag[0]=.995d0

        ENDIF
        
        history[jj,*]     = [Phig[0],Phig[1],Phig[2],Bg[0],Bg[1],Bg[2],gammag[0]];alphag[0]];,alphag[1],alphag[2]]
        lateconv          = converg
       ;if converg EQ 1 then read,pause
                                ;PRINT,jj,err[jj]
        jj = jj+1
        
    ENDWHILE

    PRINT,'convergence',lateconv,gammag[0]
    ...
    error[repet,6]    = (SQRT(2.d0*ABS(1.d0-gammag[0]))-alpha[0])/alpha[0]
    ;error[repet,6]    = (alphag[0]-alpha[0])/alpha[0]
    ;error[repet,7]    = (alphag[1]-alpha[1])/alpha[1]
    ;error[repet,8]    = (alphag[2]-alpha[2])/alpha[2]
    ...
    IF(repet MOD 20 EQ 0) THEN BEGIN
        ...
        plot,error[*,6],charsize=1.5,tit=STRING(SIGMA(error[*,6]))
        ;plot,error[*,7],charsize=1.5,tit=STRING(SIGMA(error[*,7]))
        ;plot,error[*,8],charsize=1.5,tit=STRING(SIGMA(error[*,8]))
    ENDIF
...
plot,error[*,6],charsize=1.5,tit=STRING(SIGMA(error[*,6]))+STRING(MEAN(error[*,6]))
;plot,error[*,7],charsize=1.5,tit=STRING(SIGMA(error[*,7]))+STRING(MEAN(error[*,7]))
;plot,error[*,8],charsize=1.5,tit=STRING(SIGMA(error[*,8]))+STRING(MEAN(error[*,8]))
