...
; NOAA 11306 OF 2011/10/2 AT 3 DIFFERENT ATMOSPHERIC HEIGHTS 
; 12:0:0_TAI to 2:0:0_TAI
; OBS_VR varies between -3550 m/s and  2530 m/s
...
PRO analysis_NOAA11306,fitonl

COMMON filter,looktot32,looktot41,looktot50

IF(fitonl EQ 1) THEN GOTO,fitonly

LCP0=fitsio_read_image("/SUM17/D254102780/S00000/trackedLCP0.fits")
LCP5=fitsio_read_image("/SUM8/D254123303/S00000/trackedLCP5.fits")
velocityLCP50 = float(LCP5-LCP0)/float(LCP5+LCP0)
LCP0=0.0
LCP5=0.0
RCP5=fitsio_read_image("/SUM15/D254125218/S00000/trackedRCP5.fits")
RCP0=fitsio_read_image("/SUM14/D254149723/S00000/trackedRCP0.fits")
velocityRCP50 = float(RCP5-RCP0)/float(RCP5+RCP0)
RCP0=0.0
RCP5=0.0
LCP1=fitsio_read_image("/SUM0/D254105010/S00000/trackedLCP1.fits")
LCP4=fitsio_read_image("/SUM2/D254121386/S00000/trackedLCP4.fits")
velocityLCP41 = float(LCP4-LCP1)/float(LCP4+LCP1)
LCP1=0.0
LCP4=0.0
RCP1=fitsio_read_image("/SUM7/D254148051/S00000/trackedRCP1.fits")
RCP4=fitsio_read_image("/SUM3/D254126674/S00000/trackedRCP4.fits")
velocityRCP41 = float(RCP4-RCP1)/float(RCP4+RCP1)
RCP1=0.0
RCP4=0.0
LCP2=fitsio_read_image("/SUM15/D254106996/S00000/trackedLCP2.fits")
LCP3=fitsio_read_image("/SUM3/D254108747/S00000/trackedLCP3.fits")
velocityLCP32 = float(LCP3-LCP2)/float(LCP3+LCP2)
LCP2=0.0
LCP3=0.0  
RCP2=fitsio_read_image("/SUM12/D254131546/S00000/trackedRCP2.fits")
RCP3=fitsio_read_image("/SUM11/D254128775/S00000/trackedRCP3.fits")
velocityRCP32 = float(RCP3-RCP2)/float(RCP3+RCP2)
RCP2=0.0
RCP3=0.0


OPENR,1,'velocitiesNOAA11306' ; file containing OBS_VR
vel=FLTARR(1121)
READF,1,vel
CLOSE,1
...
SAVE,vel,caltot32,caltot41,caltot50,velocityLCP50,velocityRCP50,velocityLCP41,velocityRCP41,velocityLCP32,velocityRCP32,FILE='NOAA11306_caltot.bin'
...
RESTORE,'NOAA11306_caltot.bin'
RESTORE,'lookup_NOAA11243.bin'; look-up table calculated by at disk center [2048,2048], with calibration11=0 and phase maps of November 2010, in doppler_different_heights.pro (RESTORE THE 11243 FILE ON PURPOSE BECAUSE I FUCKED UP IN doppler_different_heights.pro)
...
SAVE,ratio,vtest,file='lookup_NOAA11306.bin'
...
writefits, 'Doppler50_NOAA11306.fits',Doppler50
writefits, 'Doppler41_NOAA11306.fits',Doppler41
writefits, 'Doppler32_NOAA11306.fits',Doppler32
writefits,'magnetic50_NOAA11306.fits',magnetic50
writefits,'magnetic41_NOAA11306.fits',magnetic41
writefits,'magnetic32_NOAA11306.fits',magnetic32
