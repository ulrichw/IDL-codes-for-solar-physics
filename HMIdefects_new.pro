...
PRO HMIdefects_new,angleout,returnedt,returnedc,returnedi
; angleout is used to tune E1 and the Michelsons and is a 5 element array
; returnedX are the quantities returned, throughput,
; contrast for the the NB, and integral of the transmission profile
...
Trtemp = ABS(Ee)^2.d0
IF i EQ 4 THEN Trtemp = ABS(COS(angleout[2])*Ee+SIN(angleout[2])*E0)^2.d0;ABS(E0)^2.d0
;IF i EQ 4 THEN BEGIN
;Etemp = E0
;E0 = (COS(angleout[2])*E0 + SIN(angleout[2])*Ee)   * exp(DCOMPLEX(0,1)*!pi/4.)
;Ee = (COS(angleout[2])*Ee - SIN(angleout[2])*Etemp)* exp(DCOMPLEX(0,1)*(-!pi/4.))
;Trtemp= ABS(E0)^2.d0
;ENDIF
...
Rs[0]           = 1.d0         ; reflectivity of perpendicular beam (COMPLEX NUMBER)
Ts[0]           = 0.0d0         ; transmissivity of perpendicular beam (COMPLEX NUMBER)
Rp[0]           = 0.0d0         ; reflectivity of parallel beam (COMPLEX NUMBER)
Tp[0]           = 1.0d0         ; transmissivity of parallel beam (COMPLEX NUMBER)
...
Eperpv = (E0v*SIN(angle1[i])   + Eev*COS(angle1[i]))*Ts[i]
Eparav = (E0v*COS(angle1[i])   - Eev*SIN(angle1[i]))*Tp[i]
...
; after second passage through beamsplitter
Eperps = (E0s*SIN(angle2[i])   + Ees*COS(angle2[i]))*Rs[i]
Eparas = (E0s*COS(angle2[i])   - Ees*SIN(angle2[i]))*Rp[i]

; after the passage through the exit quarter-wave plate
E0    = ((Eperps+Eperpv)*SIN(angle3[i]) + (Eparas+Eparav)*COS(angle3[i])) * exp( DCOMPLEX(0,1) * retardquartM[i])
Ee    =(-(Eparas+Eparav)*SIN(angle3[i]) + (Eperps+Eperpv)*COS(angle3[i])) * exp(-DCOMPLEX(0,1) * retardquartM[i])
...
returnedi=TOTAL(Tr)*(l[1]-l[0])*1.d7 ; integral of the transmission profile
...
; to plot the I-ripple
;nlam =4000 & lam = (findgen(nlam)-nlam/2.)*.5d-3 & a=where(abs(lam) le 2*0.172) & intensity=fltarr(200) & for i=0,199 do begin & II=thru[i]*(1.0+con[i]*cos(2.0*!pi*lam[a]/0.172+float(i)*!pi/100.))/2.0 & intensity[i]=TOTAL(II) & endfor & plot,intensity
