...
FSR[1]  = 0.33695;0.344d0-0.00207683d0;0.3421506d0  ; for the broad-band  Michelson
...
;nlam          = 501   ; number of wavelength points (ODD NUMBER)
;dlam          = 0.004 ; sampling rate in Angstroms
;dlam          = 0.008 ; sampling rate in Angstroms
...
;phases=READFITS("PHASE_MAPS_707119.fits") ; data from 2007/10/14, taken in vacuum and sunlight
;phases=READFITS("PHASE_MAPS_3292550.fits")
...
;filters     = FLTARR(nlam,ntune,nx2,nx2)
filters     = FLTARR(nlam,ntune)
;lyot        = FLTARR(nlam,nx2,nx2)
lyot        = FLTARR(nlam)

openw,lun1,'HMIfilter.'+strtrim(nx2,2)+'.out',/get_lun

FOR iii=0,nx2-1 DO BEGIN
    PRINT,iii
    FOR jjj=0,nx2-1 DO BEGIN
        ;lyot[*,iii,jjj] = blocker
        lyot = blocker
        ;NON-TUNABLE ELEMENTS
        FOR i = 0,3 DO BEGIN
;            lyot[*,iii,jjj] = lyot[*,iii,jjj]*(1.d0+BG[iii,jjj,i+3]*COS(2.d0*!pi/FSR[i+3]*lam+PHIG[iii,jjj,i+3]*!pi/180.d0))/2.d0
            lyot = lyot*(1.d0+BG[iii,jjj,i+3]*COS(2.d0*!pi/FSR[i+3]*lam+PHIG[iii,jjj,i+3]*!pi/180.d0))/2.d0
        ENDFOR
        ;TUNABLE ELEMENTS
        FOR itune = 0,ntune-1 DO BEGIN
;            filters[*,itune,iii,jjj] = lyot[*,iii,jjj]*(1.d0+contrast[0]*COS(2.d0*!pi/FSR[0]*lam[*]+PHIG[iii,jjj,0]*!pi/180.d0+HCMNBphase[itune]))/2.d0*(1.d0+contrast[1]*COS(2.d0*!dpi/FSR[1]*lam[*]+PHIG[iii,jjj,1]*!dpi/180.d0+HCMWBphase[itune]))/2.d0*(1.d0+contrast[2]*COS(2.d0*!pi/FSR[2]*lam[*]+PHIG[iii,jjj,2]*!pi/180.d0-HCME1phase[itune]))/2.d0
...
; TO READ THE FILTER PROFILES:
;filter=fltarr(nlam,6,nx2,nx2)
;openr,1,'HMIfilter.nx2.out'
;readu,1,filter
;close,1
