...
; FSR[0]]
; NB: THIS CODE IS BETTER THAN HMI_lamp_old.pro
;
; THIS PROGRAM TAKES CARE OF THE BAD EXPOSURES AND OVERSCANS
;
; ver 1.3 June 13, 2006
; modified in June 2009
;
;----------------------------------------------------------------------
...
RESTORE,'./CPT/SEQUENCE_listSun070905_560718_256.BIN'  ;OBSMODE
...
FOR jjj=0,ny-1 DO BEGIN
    ...
    !p.multi=[0,4,3]
    FOR i=0,5 DO BEGIN
        TVIM,proxy1[*,*,i],/scale,pcharsize=2;,range=[-0.025,0.02],/scale
        TVIM,proxy2[*,*,i],/scale,pcharsize=2;,range=[-0.025,0.02],/scale
    ENDFOR
   ;TVIM,I0g,/scale
    ...
    FOR iii=0,nx-1 DO BEGIN
        ...
        IF(distance[iii,jjj] LE anglim) THEN BEGIN
            ...
            FOR j=0,nseq-1 DO BEGIN 
                ...
                FOR ii=1,nparam/2 DO BEGIN
                    dIdp1[ii-1,j] = TOTAL(profileg1[*,j] *  COS(dpi*lam*ii/4.d0/FSR[0])  )
                    dIdp2[ii-1,j] = TOTAL(profileg1[*,j] *  SIN(dpi*lam*ii/4.d0/FSR[0])  )
                ENDFOR
            ENDFOR
            ...
            WHILE (converg EQ 0) DO BEGIN
                ...
                FOR j=0,nseq-1   DO BEGIN
                    ...
                    FOR i=1,nparam/2  DO profileg = profileg + proxy1[iii,jjj,i-1]*COS(dpi*lam*i/4.d0/FSR[0])+proxy2[iii,jjj,i-1]*SIN(dpi*lam*i/4.d0/FSR[0])
...
;SAVE,proxy1,proxy2,center,I0g,FILE='RESULTS/RESULTS_LAMP_561378_'+STRTRIM(STRING(LONG(nx)),1)+'.BIN'
SAVE,proxy1,proxy2,center,I0g,FILE='RESULTS/RESULTS_LAMP_560718_'+STRTRIM(STRING(LONG(nx)),1)+'.BIN'
PRINT,'TIME=',SYSTIME(1)-time0

draw:

;RESTORE,'RESULTS/RESULTS_LAMP_561378_'+STRTRIM(STRING(LONG(nx)),1)+'.BIN'
RESTORE,'RESULTS/RESULTS_LAMP_560718_'+STRTRIM(STRING(LONG(nx)),1)+'.BIN'
...
FOR i=0,6 DO BEGIN
    TVIM,proxy1[*,*,i]*distance,range=[-0.025,0.02],/scale,tit=STRING(i+1)+'/(4. FSR!DNB!N)'
    TVIM,proxy2[*,*,i]*distance,range=[-0.025,0.02],/scale,tit=STRING(i+1)+'/(4. FSR!DNB!N)'
ENDFOR
DEVICE,/close
...
!P.MULTI=[0,2,3]
loadct,4
DEVICE,file='yo2b.ps',xoffset=-0.8,yoffset=0,xsize=22,ysize=27,/color,bits=24
